...
const TrainingDashboard = () => {
  ...
  const markVideoAsComplete = async (videoId, trainingId, moduleId) => {
    try {
      ...
      const response = await lmswebAPI.markVideoAsComplete(
        videoId,
        trainingId,
        moduleId,
        userId
                ...
                modules: training.modules.map((module) => {
                  if (module._id === moduleId) {
                    return {
                      ...module,
                      videos: module.videos.map((video) =>
                        video._id === videoId ? { ...video, completed: true } : video
                      ),
                    };
                  }
                  return module;
  ...
  const canWatchVideo = (video, module) => {
    ...
    const previousVideo = module.videos[videoIndex - 1];
    return previousVideo && previousVideo.completed;
  };
  ...
  const canWatchModule = (module, moduleIndex) => {
    ...
    const previousModule = selectedTraining?.modules?.[moduleIndex - 1];
    if (!previousModule || !previousModule.videos) return false;
    return previousModule.videos.every((video) => video.completed);
  };
  ...
  const watchVideo = (video) => {
    ...
    const videoWithTrainingId = { ...video, trainingId: selectedTraining?._id };
    setIsVideoWatched(false);
    setVideoWatchTime(0);
    setVideoDuration(0);
    setCurrentVideo(videoWithTrainingId);
  ...
  const toggleModuleExpansion = (moduleId) => {
    setSelectedTraining((prev) => {
      if (!prev) return null;
      const updatedModules = prev.modules.map((module) =>
        module._id === moduleId ? { ...module, expanded: !module.expanded } : module
      );
      return { ...prev, modules: updatedModules };
    });
  };
  ...
  const openTrainingDetails = async (training) => {
    try {
      const trainingDetails = await fetchTrainingDetails(training.trainingId);
      if (trainingDetails) {
        const modulesWithExpanded = (trainingDetails.modules || []).map((m) => ({
          ...m,
          expanded: true,
        }));
        setSelectedTraining({
  ...
  const extractYouTubeVideoId = (url) => {
    ...
    else if (url.includes('youtube.com/watch?v=')) videoId = url.split('v=')[1];
    ...
    if (showVideoModal && currentVideo && currentVideo.videoUrl) {
      ...
      if (videoId && window.YT && window.YT.Player) {
        const player = new window.YT.Player('youtube-player', {
          ...
          videoId,
          playerVars: {
            controls: 0, // Disable all controls including fast-forward
            disablekb: 1, // Disable keyboard controls
            fs: 0, // Disable fullscreen
            rel: 0, // Don't show related videos
            showinfo: 0, // Don't show video info
            modestbranding: 0, // Remove YouTube branding completely
            iv_load_policy: 3, // Don't show annotations
            cc_load_policy: 0, // Don't show captions
            playsinline: 1, // Play inline on mobile
            enablejsapi: 1, // Enable JavaScript API
            ...
            wmode: 'transparent', // Make background transparent
            allowfullscreen: false, // Disable fullscreen
              ...
              onStateChange: (event) => {
                ...
                if (state === window.YT.PlayerState.PLAYING) {
                  ...
                  // If user tries to skip more than 5 seconds, reset to beginning
                  if (currentTime > 5 && videoWatchTime < 5) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (videoPlayer && videoPlayer.getCurrentTime) {
        ...
        setVideoWatchTime(currentTime);
        const watchPercentage = (currentTime / videoDuration) * 100;
        if (watchPercentage >= 95) setIsVideoWatched(true);
      }
    }, 1000);
    return () => clearInterval(interval);
  }, [videoPlayer, videoDuration]);
  ...
  const completeVideo = async () => {
    ...
    if (!isVideoWatched) {
      alert(
        'Please watch the complete video before marking it as complete. You must watch at least 95% of the video.'
      );
      return;
    }
    ...
    try {
      ...
      if (
        ...
        assessmentResponse.data.questions &&
        assessmentResponse.data.questions.length > 0
      ) {
        setShowAssessmentModal(true);
        return;
      }
      ...
      // Try to recover a mismatched id
      let foundVideoId = null;
      if (selectedTraining && selectedTraining.modules) {
        for (const module of selectedTraining.modules) {
          if (module.videos) {
            const mv = module.videos.find(
              (v) =>
                v.title === currentVideo.title ||
                v.videoUri === currentVideo.videoUrl ||
                v.videoUri === currentVideo.videoUri
      ...
      if (foundVideoId && foundVideoId !== currentVideo._id) {
        try {
          ...
          if (
            correctedResponse &&
            correctedResponse.success &&
            correctedResponse.data &&
            correctedResponse.data.questions &&
            correctedResponse.data.questions.length > 0
          ) {
            setCurrentVideo((prev) => ({ ...prev, _id: foundVideoId }));
            setShowAssessmentModal(true);
            return;
  ...
  const currentTrainings =
    activeTab === 'assigned' ? userModules : mandatoryTrainings;
    ...
    <div className="training-dashboard">
              ...
              {selectedTraining.modules && selectedTraining.modules.length > 0 ? (
                selectedTraining.modules.map((module, moduleIndex) => {
                  ...
                  return (
                    <div
                      key={module._id}
                      className={`module-content ${!isModuleUnlocked ? 'locked-module' : ''}`}
                    >
                      <div
                        className="module-header"
                        onClick={() => (isModuleUnlocked ? toggleModuleExpansion(module._id) : null)}
                      >
                        ...
                        {isModuleUnlocked && (
                          <span className="expand-icon">{module.expanded ? '−' : '+'}</span>
                        )}
                        {!isModuleUnlocked && <span className="locked-icon">🔒</span>}
                      </div>
                      ...
                      {module.expanded && isModuleUnlocked && (
                        <div className="module-videos">
                          {module.videos && module.videos.length > 0 ? (
                            module.videos.map((video, index) => (
                              <div key={video._id} className={`video-item ${video.completed ? 'completed' : ''}`}>
                                <div className="video-info">
                                  <h4>
                                    {video.title}
                                    {index === 0 && <span className="first-video-badge">First Video</span>}
      ...
      {showAssessmentModal && currentVideo && (
        <VideoAssessmentModal
          ...
          trainingId={currentVideo.trainingId}
          moduleId={
            selectedTraining?.modules?.find((m) =>
              m.videos?.find((v) => v._id === currentVideo._id)
            )?._id
          }
        />
